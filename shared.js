...
// ═══════════════════════════════════════════
//  Share result — generates a 1080×1080 share card and triggers
//  the native share sheet (Web Share API) with the image blob.
//  Falls back to downloading the image if sharing isn't supported.
//  Each game calls this with its own data at end-of-game.
// ═══════════════════════════════════════════
function pkShareResult(data){
  const {
    gameName    = 'PIXEL KITCHEN',
    gameEmoji   = '🍳',
    themeColor  = '#f5c842',
    score       = 0,
    stars       = 0,
  ...
  // Background — radial from theme color → near black
  const g = ctx.createRadialGradient(S/2, S*0.38, 40, S/2, S*0.5, S*0.8);
  g.addColorStop(0, themeColor + '33');
  g.addColorStop(1, '#050200');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, S, S);
  // Solid backing to keep colors punchy
  ctx.fillStyle = 'rgba(10,6,20,0.78)';
  ctx.fillRect(0, 0, S, S);

  // Thick themed border + inner accent
  ctx.strokeStyle = themeColor; ctx.lineWidth = 10;
  ctx.strokeRect(24, 24, S-48, S-48);
  ctx.strokeStyle = themeColor + '55'; ctx.lineWidth = 2;
  ...
  const bracket = (x, y, fx, fy) => {
    ...
    ctx.beginPath();
    ctx.moveTo(x, y + fy*60); ctx.lineTo(x, y); ctx.lineTo(x + fx*60, y);
    ctx.stroke();
  };
  bracket(72, 72, 1, 1);  bracket(S-72, 72, -1, 1);
  bracket(72, S-72, 1, -1); bracket(S-72, S-72, -1, -1);
  ...
  ctx.shadowColor = '#8b3a00'; ctx.shadowBlur = 18;
  ctx.fillText('★ PIXEL KITCHEN ★', S/2, 140);
  ctx.shadowBlur = 0;

  // Big game emoji (native font — browser fallback renders colour emoji)
  ctx.font = '240px "Apple Color Emoji", "Segoe UI Emoji", serif';
  ctx.fillText(gameEmoji, S/2, 400);

  // Game name
  ctx.fillStyle = themeColor;
  ctx.font = 'bold 52px "Press Start 2P", monospace';
  ctx.fillText(gameName, S/2, 490);

  // Score label
  ctx.fillStyle = '#8a8a8a';
  ctx.font = 'bold 28px "Press Start 2P", monospace';
  ctx.fillText('SCORE', S/2, 570);

  // Score value — huge
  ctx.fillStyle = '#ffffff';
  ctx.font = 'bold 180px "Press Start 2P", monospace';
  ctx.shadowColor = themeColor; ctx.shadowBlur = 28;
  ctx.fillText(String(score), S/2, 720);
  ctx.shadowBlur = 0;

  // Stars
  if(stars > 0){
    ctx.font = '72px serif';
    const st = '★'.repeat(stars) + '☆'.repeat(Math.max(0, 3-stars));
    ctx.fillStyle = '#ffd560';
    ctx.fillText(st, S/2, 800);
  }
  ...
  if(difficulty){
    ...
    ctx.fillText('— ' + difficulty + ' —', S/2, 860);
  }

  // Subtitle
  if(subtitle){
    ctx.fillStyle = '#c0a8d4';
    ctx.font = '28px "Press Start 2P", monospace';
    ctx.fillText(subtitle, S/2, 910);
  }

  // Chef name + url
  const name = (localStorage.getItem('pkName') || 'CHEF').toUpperCase();
  ctx.fillStyle = '#f5c842';
  ctx.font = 'bold 28px "Press Start 2P", monospace';
  ctx.fillText('👨‍🍳 CHEF: ' + name, S/2, 980);

  ctx.fillStyle = '#664420';
  ctx.font = '22px "Press Start 2P", monospace';
  ctx.fillText('toh000525-coder.github.io/pixel_kitchen', S/2, 1030);

  // Ship
  const fname = 'pixel-kitchen-' + gameName.toLowerCase().replace(/[^a-z0-9]+/g,'-') + '.png';
  canvas.toBlob(async (blob) => {
    ...
    const shareData = {
      ...
      text: 'I scored ' + score + ' in ' + gameName + '! Try Pixel Kitchen 🍳',
    };
    try {
    ...
    // Fallback: open the image in a new tab so user can save it
    try {
...
// Each game sets window._pkLastResult at end-of-game and calls pkShareLast()
// from its SHARE button.
function pkShareLast(){
  if(window._pkLastResult) pkShareResult(window._pkLastResult);
}
